...
/// Anemoi protocol constants
pub struct AnemoiConstants<F: PrimeField> {
    pub generator: F,
    pub generator_inv: F,
    pub generator_square_plus_one: F,
    pub alpha_inv: [u64; 4],
}

impl<F: PrimeField> AnemoiConstants<F> {
    pub fn new() -> Self {
        Self {
            generator: F::from(5u64),
            generator_inv: F::from_str_vartime(
                "8755297148735710088898562298102910035419345760166413737479281674630323398247",
            )
            .unwrap_or(F::ZERO),
            generator_square_plus_one: F::from(26u64),
            alpha_inv: [
...
/// Anemoi round keys
pub struct AnemoiRoundKeys<F: PrimeField> {
    pub round_keys_x: Vec<[F; 2]>,
    pub round_keys_y: Vec<[F; 2]>,
    pub preprocessed_round_keys_x: Vec<[F; 2]>,
    pub preprocessed_round_keys_y: Vec<[F; 2]>,
}

impl<F: PrimeField> AnemoiRoundKeys<F> {
    pub fn new() -> Self {
        // Round keys string constants (extracted to avoid duplication)
        ...
        Self {
            round_keys_x: Self::convert_string_keys(&round_keys_x_strings),
            round_keys_y: Self::convert_string_keys(&round_keys_y_strings),
            preprocessed_round_keys_x: Self::convert_string_keys(&preprocessed_round_keys_x_strings),
            preprocessed_round_keys_y: Self::convert_string_keys(&preprocessed_round_keys_y_strings),
        }
    }

    fn convert_string_keys(string_keys: &[[&str; 2]; 14]) -> Vec<[F; 2]> {
        string_keys
            .iter()
            .map(|round_key| {
                [
...
/// Initialize witness polynomials with zeros
fn init_witness_polynomials<F: PrimeField>(size: usize) -> (Vec<F>, Vec<F>, Vec<F>, Vec<F>, Vec<F>) {
    (
        vec![F::ZERO; size],
        vec![F::ZERO; size],
        vec![F::ZERO; size],
...
/// Initialize selector polynomials with zeros
fn init_selector_polynomials<F: PrimeField>(size: usize) -> [Vec<F>; 14] {
    [(); 14].map(|_| vec![F::ZERO; size])
}

/// Apply a single Anemoi round transformation
fn apply_anemoi_round<F: PrimeField>(
    state_x: &mut [F; 2],
    state_y: &mut [F; 2],
    round: usize,
    ...
    state_x[0] += round_keys.round_keys_x[round][0];
    state_x[1] += round_keys.round_keys_x[round][1];
    state_y[0] += round_keys.round_keys_y[round][0];
    state_y[1] += round_keys.round_keys_y[round][1];

    // Step 2: MDS matrix application
    let temp_x0 = state_x[0] + constants.generator * state_x[1];
    let temp_x1 = constants.generator * state_x[0] + constants.generator_square_plus_one * state_x[1];
    state_x[0] = temp_x0;
    state_x[1] = temp_x1;

    // MDS to y with word permutation (y[1], y[0] -> y[0], y[1])
    let temp_y0 = state_y[1] + constants.generator * state_y[0];
    let temp_y1 = constants.generator * state_y[1] + constants.generator_square_plus_one * state_y[0];
    state_y[0] = temp_y0;
    state_y[1] = temp_y1;
    ...
    for i in 0..2 {
        state_x[i] -= constants.generator * state_y[i].square();
        state_y[i] -= state_x[i].pow(constants.alpha_inv);
        state_x[i] += constants.generator * state_y[i].square() + constants.generator_inv;
    }
}

/// Apply final MDS and PHT transformations
fn apply_final_transformations<F: PrimeField>(
    state_x: &mut [F; 2],
    state_y: &mut [F; 2],
    constants: &AnemoiConstants<F>,
) {
...
/// Create permutation cycles for witness polynomials
fn create_witness_permutation_cycles(poly_indices: &[usize], gate_idx: usize) -> Permutation {
    let mut permutation = Permutation::default();
    for &poly_idx in poly_indices {
        permutation.copy((poly_idx, gate_idx), (poly_idx, gate_idx));
    }
    permutation
}

/// Build a complete Jive CRH circuit for Merkle tree level
fn build_jive_crh_circuit<F: PrimeField>(
    left_hash: F,
    middle_hash: F,
    right_hash: F,
    level_padding: F,
...
pub fn rand_anemoi_hash_circuit_with_flatten<F: PrimeField, R: Rotatable + From<usize>>(
    ...
    let generator_inv = F::from_str_vartime(
        "8755297148735710088898562298102910035419345760166413737479281674630323398247",
    )
    .unwrap_or(F::ZERO);
...
/// tree_location verification gates for Merkle proof
