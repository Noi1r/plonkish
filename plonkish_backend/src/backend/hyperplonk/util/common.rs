...
// ==================== Common Constants and Helper Structures ====================
...
pub fn init_witness_polynomials<F: PrimeField>(size: usize) -> (Vec<F>, Vec<F>, Vec<F>, Vec<F>, Vec<F>) {
    (
        vec![F::ZERO; size],
        vec![F::ZERO; size],
        vec![F::ZERO; size],
...
pub fn init_selector_polynomials<F: PrimeField>(size: usize) -> [Vec<F>; 14] {
    [(); 14].map(|_| vec![F::ZERO; size])
}

/// Apply a single Anemoi round transformation
pub fn apply_anemoi_round<F: PrimeField>(
    state_x: &mut [F; 2],
    state_y: &mut [F; 2],
    round: usize,
...
pub fn apply_final_transformations<F: PrimeField>(
    state_x: &mut [F; 2],
    state_y: &mut [F; 2],
    constants: &AnemoiConstants<F>,
) {
...
pub fn generate_padding_constants<F: PrimeField>(num_levels: usize) -> Vec<F> {
    ...
    (0..num_levels)
        .map(|_| F::ZERO) // Simple deterministic pattern
        .collect()
}

/// Permutation structure for managing copy constraints
#[derive(Default)]
...
pub trait ExpressionExt<F: Field> {
